...
public class TransactionDAO {
    ...
    public boolean addTransaction(int userId, LocalDate date, double amount, String type, int categoryId, boolean isFuture, boolean isExtraordinary) {
    ...
    public List<Transaction> getFilteredTransactions(int userId, LocalDate startDate, LocalDate endDate, Integer categoryId, String type) {
        ...
        StringBuilder sql = new StringBuilder(
            ...
            "WHERE t.user_id = ?"); // ユーザーIDでフィルタリング
    ...
    public double calculateCurrentBalance(int userId) {
        String sql = "SELECT " +
                     ...
                     "FROM transactions WHERE user_id = ? AND is_future = 0";
    ...
    public List<MonthlySummary> getMonthlySummary(int userId, LocalDate startDate, LocalDate endDate) {
        ...
        if (startDate != null) sql += " AND date >= ? ";
        if (endDate != null) sql += " AND date <= ? ";
        ...
        try (Connection conn = DBManager.connect();
            ...
            if (startDate != null) pstmt.setString(paramIndex++, startDate.toString());
            if (endDate != null) pstmt.setString(paramIndex++, endDate.toString());

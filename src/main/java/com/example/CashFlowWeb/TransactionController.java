...
public class TransactionController {
    ...
    public List<MonthlySummary> getMonthlySummary(@AuthenticationPrincipal UserDetails userDetails) {
        ...
        // 月ごとに収入と支出を集計
        Map<String, double[]> aggMap = new TreeMap<>(); // Key: YYYY-MM, Value: [income, expense]
     ...
     * ★修正点: 目標未達時に「Xヶ月遅れる可能性があります」と具体的に警告するように変更
     */
    @GetMapping("/predict")
        ...
        if (goals.isEmpty()) {
            ...
            if (futureValueAtTarget >= targetAmount) {
                ...
                if (averageMonthlyProfit > 0) {
                    ...
                    // 利益が出ていない場合、達成不能として扱う
                    delayMonths = 999; 
                }

                // ★ メッセージ生成ロジックの修正箇所 ★
                String moneyFormat = String.format("%,.0f", shortfallAmount);
                if (delayMonths > 0 && delayMonths < 600) { // 50年以上の遅れは実質無理とみなす
                    feedback = "目標「" + targetGoal.getName() + "」の達成は" + delayMonths + "ヶ月遅れる可能性があります。" +
                               "（不足額: 約" + moneyFormat + "円）" +
                               " 支出を見直すか、収入を増やすことを検討してください。";
                } else {
                    feedback = "目標「" + targetGoal.getName() + "」の達成は現在の収支状況では困難です。" +
                               "（不足額: 約" + moneyFormat + "円）" +
                               " 抜本的な収支の見直しが必要です。";
                }
            }

...
public class TransactionController {
    ...
    // 6. 取引追加（ユーザーIDを紐付け）
    @PostMapping
    public ResponseEntity<Void> addTransaction(@AuthenticationPrincipal User user, @RequestBody Transaction transaction) {
    ...
    public ResponseEntity<PredictionResult> predictGoalAchievement(@AuthenticationPrincipal User user) {
        ...
        List<Transaction> recentTransactions = transactionDAO.getFilteredTransactions(user.getId(), threeMonthsAgo, LocalDate.now(), null, null);
        
        double totalIncome = 0;
        double totalExpense = 0;
        for (Transaction t : recentTransactions) {
            if (t.getIsExtraordinary()) continue;
